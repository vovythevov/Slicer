...
//      std::cout << "\tset current index via the glyph type " << pointDisplayNode->GetGlyphType() << ", current index = " << ui.pointGlyphTypeComboBox->currentIndex() << std::endl;
  ...
  if (!lineDisplayNode)
    ...
    // tick spacing
    ui.lineTickSpacingSpinBox->setMRMLScene(this->m_logic->GetMRMLScene());
    ui.lineTickSpacingSpinBox->setValue(lineDisplayNode->GetTickSpacing());
...
void qSlicerAnnotationModulePropertyDialog::createConnection()
{
  ...
  this->connect(ui.lineTickSpacingSpinBox, SIGNAL(valueChanged(QString)),
...
void qSlicerAnnotationModulePropertyDialog::onLineTickSpacingChanged()
{
  ...
  double value = ui.lineTickSpacingSpinBox->value();
  if (lineDisplayNode->GetScene())
    {
    lineDisplayNode->GetScene()->SaveStateForUndo(lineDisplayNode);
    }
...
void qSlicerAnnotationModulePropertyDialog::lockUnlockInterface(bool lock)
{
  ...
  ui.lineTickSpacingSpinBox->setEnabled(lock);
